...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    // Ak je povoleny zapis, tak ho zakazem a nastavim PTE na COW
    if(*pte & PTE_W) {
        *pte ^= PTE_W;
        *pte |= PTE_COW;
    }
    ...
    // ak nezlyha mapovanie, tak zvysim pocet referencii na PA
    inc_ref((void*)pa);
  }
  return 0;
...
int uvmcow(pagetable_t pagetable, uint64 va)
{
    ...
    // ak PTE neexistuje
    if (pte == 0)
        panic("uvmcow: pte should exists");
    // ak PTE zaznam nie je validny
    if((*pte && PTE_V) == 0)
        panic("uvmcow: page not valid");
    // Ak PTE_COW nie je nastaveny
    if(!(*pte & PTE_COW)) {
        printf("uvmcow: PTE_COW not set for 0x%p\n", va);
        return -2;
    }

    // Ak som sa dostal sem, tak bit na PTW_COW je 1 a PTE_W 0
    // Potrebujem povolit zapis a vymazat COW
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte);
    flags ^= PTE_COW;
    flags |= PTE_W;
